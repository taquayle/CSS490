...
gEngine.ResourceMap = (function () {
    ...
    var mImgNum = 0;
    var mNames = ["assets/minion_sprite.png", 
    ...
    */
    ...
    var loadImage = function(fileName)
    {
        if (!(isAssetLoaded(fileName)))
        {
            ...
            var c = mNames[mImgNum];
            //document.getElementById("debug").innerHTML = mNames[mImgNum] + " " + mNames.length + " " + mImgNum;
            asyncLoadCompleted(fileName, c);
        }
    };
    
    var unloadImage = function(fileName)
    {
        gEngine.ResourceMap.unloadAsset(fileName);
        mImgNum = mImgNum + 1;
        if(mImgNum >= mNames.length)
            mImgNum = 0;
    };
    //</editor-fold>
